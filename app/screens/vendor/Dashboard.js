  ...
  _buildWeddings() {
    ...
    Q.all(getWeddings).then((weddingDocs) => {
      const getTodos = weddingDocs.map((weddingDoc) => {
        ...
        let weddingDate = moment(weddingData.date, 'MM/DD/YYYY')
        let bg2 = undefined
        ...
        todoSnapshot.forEach((todoDocs) => {
          ...
          todoDocs.forEach((doc) => {
            ...
            if(weddingDates[weddingId] !== undefined) {
              dueDate = moment(weddingDates[weddingId], 'MM/DD/YYYY').subtract(todo.days_before_wedding, 'days')
            }
              ...
              rightComponent={
                <View style={styles.iconWrapper}>
                  <View style={styles.iconContainer}>
                    <TouchableOpacity
                      onPress={() => this.props.navigation.navigate(item.editScreen, { todoId: item.id })}
